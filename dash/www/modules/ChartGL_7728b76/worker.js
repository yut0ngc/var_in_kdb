...
onmessage = function(e) {
    ...
    if (e.data == "queue") {
        ...
        const axisType = e.data[2];
        const isBounds = e.data[3];
        ...
        if (LONG_TYPES.indexOf(colType) !== -1 && isIE11) {
            ...
            if (axisType === "Linear" || axisType === "Time") {
                ...
                const valuesObj = new Map();
                colData.forEach((val, i) => {
                    const objVal = valuesObj.get(val);
                    if (!objVal) valuesObj.set(val, [i]);
                    else objVal.push(i);
                });
                indices = [].concat.apply([], Array.from(valuesObj.values()));
            }
        }
        postMessage(indices);
    }
